...
test: ## Run all the tests
	go test -short -cover -race -timeout=60s $$(go list ./... | grep -Ev 'vendor|mocks')

.PHONY: testc
testc: ## Run all the tests with coverage
	echo 'mode: atomic' > coverage.txt && \
		go test -covermode=atomic -coverprofile=coverage.txt -v -race -timeout=30s \
		$$(go list ./... | grep -Ev 'vendor|mocks')

.PHONY: cover
cover: testc ## Run all the tests and opens the coverage report
